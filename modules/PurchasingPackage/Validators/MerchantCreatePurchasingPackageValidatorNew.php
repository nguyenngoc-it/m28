...
class MerchantCreatePurchasingPackageValidatorNew extends Validator
{
    /**
        ...
        if (
            ...
            (
                PurchasingPackage::query()->where('freight_bill_code', trim($this->input['freight_bill_code']))
                    ->where('merchant_id', $this->merchant->id)->count() > 0
            )
        ) {
    ...
    protected function validateMerchant()
    {
        ...
        $this->merchant = Merchant::query()->where(['code' => $merchantCode])->where('tenant_id', $this->user->tenant->id)->first();
        if (!$this->merchant) {
