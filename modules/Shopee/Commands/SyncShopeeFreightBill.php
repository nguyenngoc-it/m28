...
class SyncShopeeFreightBill
{
     ...
     * @param User $creator
     */
    public function __construct($shopId, $orderCode, $trackingNo, User $creator = null)
    {
        $this->shopId     = $shopId;
        ...
        $this->logger = LogService::logger('shopee', [
            'context' => ['shop_id' => $shopId, 'order_code' => $orderCode, 'tracking_no' => $trackingNo],
        ]);
    }
    ...
    /**
     * @param Order $order
     * @return FreightBill|null|object
     * @throws RestApiException
     * @throws MarketplaceException
     */
    protected function sync(Order $order)
    {
        ...
        $this->syncFreightBillStatus($order, $freightBill);
    ...
    protected function createFreightBill(OrderPacking $orderPacking)
    {
        ...
        dispatch(new ShopeeCreateShippingDocumentJob($orderPacking->order_id));
     ...
     * @throws MarketplaceException
     */
    protected function syncFreightBillStatus(Order $order, FreightBill $freightBill)
    {
        $logisticsStatus = $this->getLogisticsStatus($order);

        if (!$status = Service::shopee()->mapFreightBillStatus($logisticsStatus)) {
     ...
     * @throws MarketplaceException
     */
    protected function getLogisticsStatus(Order $order)
    {
        $trackingInfo = $order->store->shopeeApi()->getTrackingInfo([
            'order_sn' => $this->orderCode,
        ])->getData();

        $this->logger->info('tracking_info '.$this->orderCode, compact('trackingInfo'));

        return (string)Arr::get($trackingInfo, 'response.logistics_status', '');
    }
}
