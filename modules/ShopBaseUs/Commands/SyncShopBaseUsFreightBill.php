...
class SyncShopBaseUsFreightBill
{
    /**
     * @var Store
     */
    ...
    protected function syncFreightBillStatus(FreightBill $freightBill)
    {
        ...
        if (!$logisticsStatus || !$status = Service::shopBaseUs()->mapFreightBillStatus($logisticsStatus)) {
    ...
    protected function getLogisticsStatus()
    {
        ...
        $fulfillments = $shopBaseUsApi->getOrderFulfillments($params)->getData('fulfillments');
        if ($fulfillments) {
           foreach ($fulfillments as $fulfillment) {
                $logisticsMessage = data_get($fulfillment, 'shipment_status');
           } 
        }
