...
class SyncShopBaseUsStock
{
    ...
    /**
     * SyncShopBaseUsStock constructor.
     * @param Sku $sku
     */
    ...
    public function handle()
    {
        ...
        foreach ($storeSkus as $storeSku) {
            ...
            try {
                ...
                $this->logger->info('updateProductShopBaseUs  ' . $storeSku->store->id, compact('paramsRequest', 'data'));
    
            } catch (\Exception $exception) {
                $this->logger->info('updateProductShopBaseUs error ' . $storeSku->store->id . ' - ' .$exception->getMessage(), compact('paramsRequest'));
            }
        }
    }
