...
class SyncKiotVietFreightBill
{
     ...
     * @param User $creator
     */
    public function __construct($store, $order, $trackingNo, User $creator = null)
    {
        $this->store      = $store;
        $this->order      = $order;
    ...
    protected function syncFreightBillStatus(FreightBill $freightBill)
    {
        ...
        if ($logisticsStatus) {
            $status = Service::kiotviet()->mapFreightBillStatus($logisticsStatus);
            if (!$status) {
    ...
    protected function getLogisticsStatus()
    {
        ...
        $check = preg_match('/HD/',$this->order->code);
        $orderType = 'order';

        if ($check) $orderType = 'invoice';

        $logisticsMessage = Service::kiotviet()->api()->getLogisticsMessage([
            'order_code'      => $this->order->code,
            'order_type'      => $orderType,
            'tracking_number' => $this->trackingNo,
            'shop_name'       => $this->store->getSetting('shop_name'),
        ], $this->store);

        $response = $logisticsMessage->getData("{$orderType}Delivery.status");
